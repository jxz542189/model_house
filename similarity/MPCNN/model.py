...
def init_weight(shape, name):
    var = tf.Varaibel(tf.truncated_normal(shape, mean=0, stddev=1.0), name=name)
    return var


class MPCNN_Layer(object):
    def __init__(self, num_classes, embedding_size, filter_sizes, num_filters, n_hidden,
        ...
        self.h =num_filters[0] * len(self.pooling) * 2 + \
            num_filters[1] * (len(self.poolings) - 1) * (len(filter_sizes) - 1) * 3 + \
            len(self.poolings) * len(filter_sizes) * len(filter_sizes) * 3
        self.Wh = tf.Variable(tf.random_normal([604, n_hidden], stddev=0.01),
                              name='Wh')
        self.bh = tf.Variable(tf.constant(0.1, shape=[n_hidden]),
                              name='bh')
        self.Wo = tf.Variable(tf.random_normal([n_hidden, num_classes],
                                               stddev=0.01),
                              name="Wo")
        self.bo = tf.Variable(tf.constant(0.1, shape=[num_classes]),
                              name='bo')
    ...
    def built_block_A(self, x):
        out = []
        with tf.name_scope("build_block_A"):
    ...
    def similarity_sentence_layer(self):
        sent1 = self.built_block_A(self.input_x1)
        sent2 = self.built_block_B(self.input_x2)
        fea_h = []
