...
# Network parameters
tf.flags.DEFINE_integer('hidden_units', 128, 'Number of hidden units in each layer')
tf.flags.DEFINE_integer('num_blocks', 1, 'Number of blocks in each attention')
tf.flags.DEFINE_integer('num_heads', 8, 'Number of heads in each attention')
tf.flags.DEFINE_float('dropout', 0.0, 'Dropout probability(0.0: no dropout)')
tf.flags.DEFINE_float('regulation_rate', 0.00005, 'L2 regulation rate')

tf.flags.DEFINE_integer('itemid_embedding_size', 64, 'Item id embedding size')
tf.flags.DEFINE_integer('cateid_embedding_size', 64, 'Cate id embedding size')

tf.flags.DEFINE_boolean('concat_time_emb', True, 'Concat time-embedding instead of Add')

# Training parameters
tf.flags.DEFINE_boolean('from_scratch', True, 'Romove model_dir, and train from scratch, default: False')
tf.flags.DEFINE_string('model_dir', 'save_path', 'Path to save model checkpoints')
tf.flags.DEFINE_string('optimizer', 'sgd', 'Optimizer for training: (adadelta, adam, rmsprop,sgd*)')
...
tf.flags.DEFINE_integer('train_batch_size', 32, 'Training Batch size')
tf.flags.DEFINE_integer('test_batch_size', 128, 'Testing Batch size')
tf.flags.DEFINE_integer('max_epochs', 10, 'Maximum # of training epochs')

tf.flags.DEFINE_integer('display_freq', 100, 'Display training status every this iteration')
tf.flags.DEFINE_integer('eval_freq', 1000, 'Display training status every this iteration')

# Runtime parameters
tf.flags.DEFINE_string('cuda_visible_devices', '0', 'Choice which GPU to use')
tf.flags.DEFINE_float('per_process_gpu_memory_fraction', 0.0, 'Gpu memory use fraction, 0.0 for allow_growth=True')
