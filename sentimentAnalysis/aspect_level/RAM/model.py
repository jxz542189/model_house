    ...
    def build_model(self):
        ...
        with tf.name_scope('dynamic_rnn'):
            ...
            def body(i, memory):
                ...
                weight = 1 - b
                memory = memory.write(i, tf.concat(
                    [tf.multiply(a, tf.tile(tf.expand_dims(weight, -1), [1, self.n_hidden * 2])),
                     tf.reshape(b, [-1, 1])], 1))
                return (i + 1, memory)
            ...
            for h in range(self.n_hop):
                ...
                def body(i, newe, score):
                    ...
                    g = tf.matmul(weights['attention'][h], tf.transpose(tf.concat([a, b, c], 1), perm=[1, 0])) + \
                        biases['attention'][h]
                    score_temp = tf.concat(
                        [tf.nn.softmax(tf.slice(g, [0, 0], [1, l])), tf.zeros([1, self.max_sentence_len - l])], 1)
                    ...
                    e0 = tf.nn.tanh(
                        tf.matmul(weights['gru_x'], i_AL) + tf.matmul(weights['gru_g'], tf.multiply(r, olde)))
            ...
            self.scores = tf.transpose(tf.reshape(tf.stack(scores_list), [self.n_hop, -1, self.max_sentence_len]),
                                       [1, 0, 2])
